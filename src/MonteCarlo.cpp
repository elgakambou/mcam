...
double MonteCarlo :: price(double T, int nbDates, PnlRng* rng)
{
    ...
    double exp_timeStep = exp(mod_->r_* timeStep);

    for (int k = nbDates - 1; k >=1 ; k--)
    {
        double exp_k_timeStep = exp(-mod_->r_ * k * timeStep);
        ...
        for (int l = 0; l<nbSamples; l++)
        {
            ...
            double curr_payoff = exp(-mod_->r_ * tau_k_l_1*timeStep) * opt_->payoff(S_tau_k_l_1); // tau_k_l_1
            pnl_mat_get_row(S_tk_l, path_l, k);
            pnl_mat_set_row(dataX, S_tk_l, l);      
            pnl_vect_set(dataY, l, curr_payoff);
        }
